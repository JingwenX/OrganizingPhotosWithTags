...
/*
 * This FileNode class uses Composite Design Pattern.
 * 
 * FileNode objects are in tree structure since every object has a list of children, 
 * which contains FileNode objects. (It contains groups of its own objects.)
 * 
 * Each of the FileNode object represents itself, and a part of the tree.
 * (The root is able to represent the whole tree structure.)
 * 
 * FileNode class also provides ways to modify its group of objects, such as 
 * method addChild, findChild (and applies other methods to it).
 * 
 */

/*
 * This FileNode class is a part of the Command Design Pattern.
 * 
...
/**
 * <h1>Stores File Information in FileNode Structure</h1> The root of a tree
 * representing a directory structure. If this root is an image, it will contain
 * the tags and names information of this image.
 * <p>
 * 
...
public class FileNode implements Serializable {
	...
	/**
	 * Finds and returns a map of oldNames and tags used in the oldNames of
	 * given FileNode.
	 * 
	 * @return a map of all the old names of a given FileNode, with the name as
	 *         the key of the map and an ArrayList of tags used in that old name
	 *         as the value.
	 */
	...
	/**
	 * This node's oldNames map, a map of oldNames and tags used in the oldNames
	 * of given FileNode. Every oldName in this map is unique, and this record
	 * is created when a name becomes an old name.
	 */
	private Map<String, ArrayList<Tag>> oldNames;
	/**
	 * A map of Date and Name of this node, mapping from the Date of creating a
	 * name to that name. A name may appear multiple times as the value for
	 * different date key.
	 */
	...
	/**
	 * A node in this tree. The file type of the node would only be IMAGE or
	 * DIRECTORY.
	 *
	 * @param name
	 *            the file
	...
	public FileNode(String name, FileNode parent, FileType type, String path) {
		...
		/* initiates log and filehandler and set level */
		/* uses try catch block to handle SecurityException and IOException */
		try {
			if (lg == null) {
		...
		if (type == FileType.IMAGE) {
			...
			/* records time and current name when node is initiated */
	...
	/**
	 * Finds and returns the map of Date and Name of this node, mapping from the
	 * Date that a name is first created to the names, including its current
	 * name.
	 * <p>
	 * 
	 ...
	 * <p>
	 * "FileNode name: this.name, type: IMAGE/DIRECTORY, parent:
	 * this.parent.name, path: this.path, tags: this.tags, oldNames:
	 * this.getDateToName, children: this.children"
	 * <p>
	 * If that is the FileNode is IMAGE type, then return without "children", if
	 * that is the FileNode is DIRECTORY type, then return without "tags". If
	 * the FileNode does not have a parent, then return with parent name being
	 * "no parent".
	 * 
	...
	public String toString() {
		if (this.parent != null) {
			if (this.type == FileType.IMAGE) {
				return "FileNode name: " + this.name + ", type: " + "IMAGE" + ", parent: " + this.parent.name
						+ ", path: " + this.path + ", tags: " + this.tags + ", oldNames: " + this.oldNames
						+ ", dateToName: " + this.getDateToName;
			} else { // this.type == FileType.DIRECTORY
				return "FileNode name: " + this.name + ", type: " + "DIRECTORY" + ", parent: " + this.parent.name
						+ ", path: " + this.getPath() + ", children: " + this.children;
			}
		} else {
			if (this.type == FileType.IMAGE) {
				return "FileNode name: " + this.name + ", type: " + "IMAGE" + ", parent: " + "no parent" + ", path: "
						+ this.path + ", tags: " + this.tags + ", oldNames: " + this.oldNames + ", dateToName: "
						+ this.getDateToName;
			} else { // this.type == FileType.DIRECTORY
				return "FileNode name: " + this.name + ", type: " + "DIRECTORY" + ", parent: " + "no parent"
						+ ", path: " + this.getPath() + ", children: " + this.children;
			}
		}
	}
	...
	/**
	 * Compares this FileNode and another FileNode and see whether they are the
	 * same based on their name and path.
	 * 
	 * @param obj
	 *            another FileNode to compare with this FileNode to check
	 *            equality.
	 * @return whether both of the name and path of this FileNode and another
	 *         FileNode are the same.
	 */
	@Override
	...
	/**
	 * Finds and returns a child node of a given name in this directory tree, or
	...
	/**
	 * Returns this node's list of Tag objects
	 * 
	 */
	...
	/**
	 * Returns this node's path of the image that the node represents.
	 * 
	 * @return this node's path of the image that the node represents.
	 */
	...
	/**
	 * Adds a childNode, representing a file or directory named name, as a child
	 * of this node.
	 * 
	 * @param name
	...
	/**
	 * Updates a childNode's name in the children map of its parent when the
	 * name of the FileNode (childNode of its parent) has been modified due to
	 * tag add/delete operation.
	 * 
	 * @param oldName
	...
	/**
	 * Updates the oldName map when the the name of a FileNode is about to
	 * change.
	 * <p>
	 * This would only be performed if this name is not currently recorded in
	 * the oldNames list of this FileNode.
	 * 
	...
	public void addOldName() {
		if (!this.oldNames.keySet().contains(this.name)) {
			/*
			 * clones the tag list that a specific old name had, so that the tag
	...
	/**
	 * Puts the current Date and current name into the getDateToName map of this
	 * FileNode.
	 * <p>
	 * This is usually used to keep track of a newly generated name when the
	 * user modify the tags of a FileNode.
	 * 
	...
	/**
	 * Adds a given tag t to the FileNode and change the name of the file
	 * accordingly only if the given tag t is not one of the tags the FileNode
	 * currently has.
	 * 
	 * @param t
	 ...
	 * @param updateOldName
	 *            whether to record this new name and the Date it is generated
	 *            to the getDateToName map of this FileNode.
	 * @see FileNode#getDateToName
	 */
	public void addTag(Tag t, Boolean logging, Boolean updateOldName) {
		if (this.type == FileType.IMAGE && !this.tags.contains(t)) {
			/* updates old name list */
			if (updateOldName) {
				addOldName();
			}
			/* changes the name of the file in the file system */
			...
			/* updates FileNode attributes */
			this.name = newName;
			this.path = newFilePath;
			/*
			 * prepends new tag to tags so that the order is the same as they
			 * appear in the name
			 */
			this.tags.add(0, t);
			/* updates dateToName */
	...
	/**
	 * Deletes the given tag t, and changes its file name accordingly.
	 * <p>
	 * Does nothing if the given tag t is not one of the tags this FileNode
	 * currently has.
	 * 
	 * @param t
	 *            the Tag to delete
	 ...
	 * @param updateOldName
	 *            whether to record this new name and the Date it is generated
	 *            to the getDateToName map and also record to OldNames map of
	 *            this FileNode.
	 * @see FileNode#getDateToName
	 */
	public void deleteTag(Tag t, Boolean logging, Boolean updateOldName) {
		if (this.type == FileType.IMAGE && this.tags.contains(t)) {
			...
			/* deletes tag from the name of the file */
			File oldFile = new File(this.path);
			String oldName = oldFile.getName();
	...
	/**
	 * Deletes all the tags this FileNode currently has, and changes its file
	 * name to its original file name (without any tag as prefix).
	 * 
	 * @param logging
	 *            whether to record this change into the log file.
	...
	public void deleteAllTags(Boolean logging, Boolean updateDateToName) {
		...
		if (!this.tags.isEmpty()) {
			...
			/* loops over the tag list and delete all of tags */
			for (Tag temp : taglist) {
				/*
				 * does not use deleteTag's logging, addOLdName, and
				 * updateDateToName
				 */
		...
		/* updates DateToName when all changes in deleteAllTags have finished */
		if (updateDateToName) {
			updateDateToName();
		}
		/* writes log */
	...
	/**
	 * Changes the name of this FileNode to an already used name toName in the
	 * oldNames list. And records this change in the log file if logging is set
	 * to true.
	 * 
	 * @param toName
	 *            the name (an already used name) that the user want to change
	 *            this FileNode to.
	 * @param logging
	 *            whether to record this change into the log file.
	 * @see FileNode#oldNames
	...
	public void revertToOldName(String toName, Boolean logging) {
		...
		String oldName = this.name.substring(0);
		/* checks if oldName is in oldName map */
